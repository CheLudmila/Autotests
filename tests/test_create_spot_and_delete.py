...
def test_create_spot_tender(login: Page):
    ...
    # === 3. Повернення до списку SPOT ===
    page.get_by_role("link", name="SPOT").nth(1).click()
    expect(page.get_by_role("navigation", name="Pagination")).to_be_visible(timeout=10000)

    # === 4. Пошук створеного тендера у списку ===
    row = page.get_by_role("row", name=re.compile(tender_name))
    expect(row).to_be_visible(timeout=10000)
    print(f"🔍 Знайдено тендер '{tender_name}' у списку")

    # === 5. Видалення тендера ===
    row.get_by_role("button").click()
    expect(page.get_by_role("button", name=re.compile("Видалити", re.IGNORECASE))).to_be_visible(timeout=5000)
    page.get_by_role("button", name=re.compile("Видалити", re.IGNORECASE)).click()

    expect(page.get_by_role("heading", name=re.compile("Видалити", re.IGNORECASE))).to_be_visible(timeout=5000)
    page.get_by_role("button", name="Так").click()

    # === 6. Перевірка, що тендер зник ===
    expect(page.get_by_text(tender_name)).not_to_be_visible(timeout=10000)
    print(f"🗑️ Тендер '{tender_name}' успішно видалено зі списку")